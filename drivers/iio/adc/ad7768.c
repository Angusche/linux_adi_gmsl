			    ...
			    int val, int val2, long info)
{
	...
	switch (info) {
	case IIO_CHAN_INFO_SAMP_FREQ:
		ret = ad7768_set_sampling_freq(indio_dev, val, chan->channel);
		break;

	case IIO_CHAN_INFO_CALIBBIAS:
		base_reg = ad7768_get_calib_reg_base(st, chan, false);
		ret = ad7768_write_calib_value(st, base_reg, val);
		break;
	...
	case IIO_CHAN_INFO_PHASE:
		...
		ret = regmap_write(st->regmap, base_reg, val);
		break;
...
static int ad7768_gpio_adev_init(struct ad7768_state *st)
{
	...
	adev = devm_auxiliary_device_create(dev, AD7768_AUX_DEV_GPIO_NAME,
					    (void *)st->chip_info->name);
	if (!adev)
...
static int ad7768_probe(struct spi_device *spi)
{
	...
	if (rev_id != AD7768_REV_ID_VAL)
		return dev_err_probe(dev, -ENODEV,
				     "Invalid revision ID - SPI communication failed\n");
